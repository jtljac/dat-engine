...
public:
    ...
    size_t pushBack(TValue value) {
        if (nextId < values.size()) return handleInnerInsert(value);
    ...
    size_t emplaceBack(Args&&... args) {
        if (nextId < values.size()) return handleInnerInsert(TValue(std::forward<Args>(args)...));
     ...
     * chops unused IDs of the end of the array.
     */
    void pack() {
        ...
        while (maxIndex < indices.size() - 1 && indices[maxIndex] == indices[maxIndex + 1] + 1) ++maxIndex;
