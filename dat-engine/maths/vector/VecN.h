...
namespace DatEngine {
    ...
    struct DatMaths::Vector<size, TComponent> {
         ...
         */
        explicit Vector(TComponent scalar) : components(size) { std::fill_n(components, size, scalar); }

        /**
         * Initialises each component using values from a list
         * @param components The list to get the components from
         */
        Vector(const std::array<TComponent, size>& components) {
            std::memcpy(this->components, components.data(), sizeof(TComponent) * size);
        };
         ...
         * @tparam TOtherSize The size of the other vector
         * @tparam TOther The type of the other vector
         * @param otherVec The other vector to source the components from
         */
        template<int TOtherSize, typename TOther>
        Vector(const Vector<TOtherSize, TOther>& otherVec) : components(size) {
        ...
        Vector& operator=(const Vector<size, TOther>& otherVec) {
            for (int i = 0; i < size; ++i) {
                components[i] = otherVec[i];
            }
        ...
        const TComponent& operator[](const size_t pos) const {
            assert(pos < size);
            return components[pos];
        }

        TComponent& operator[](const size_t pos) {
            assert(pos < size);
            return components[pos];
        }
        ...
        // Add Vector
        template<typename TOther>
        Vector operator+(const Vector<size, TOther>& rhs) const {
            Vector newVec;
            for (int i = 0; i < size; ++i) {
        ...
        // Add Vector In-Place
        template<typename TOther>
        Vector& operator+=(const Vector<size, TOther>& rhs) {
            for (int i = 0; i < size; ++i) {
                (*this)[i] += rhs[i];
            }
         ...
         */
        template<typename TOther>
        TComponent dotProduct(Vector<size, TOther> otherVec) const {
            TComponent total = 0;
         ...
         */
        TComponent length() const { return DatMaths::sqrt(lengthSquared()); }
        ...
        // Equals
        template<typename TOther>
        bool operator==(const Vector<size, TOther>& rhs) const {
            for (int i = 0; i < size; ++i) {
                if ((*this)[i] != rhs[i])
                    return false;
            }
        ...
        template<typename TOther>
        bool
        equal(const Vector<size, TOther>& rhs,
              TComponent tolerance = static_cast<TComponent>(constants::tinyNumber)) const {
            for (int i = 0; i < size; ++i) {
                if (std::abs((*this)[i] - static_cast<TComponent>(rhs[i])) >= tolerance)
                    return false;
            }
        ...
        // UFO Operator
        template<typename TOther>
        std::strong_ordering operator<=>(const Vector<size, TOther>& rhs) const {
            return this->lengthSquared() <=> static_cast<TComponent>(rhs.lengthSquared());
        }
    };
} // namespace DatEngine
