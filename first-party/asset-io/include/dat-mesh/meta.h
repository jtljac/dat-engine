...
namespace DatAssetIO::DatMesh {
    static constexpr uint8_t FILE_SIGNATURE[]{0xB1, 0x44, 0x41, 0x54, 0x4D, 0x45, 0x53, 0x48}; // ±DATMESH
    static constexpr uint8_t FILE_VERSION = 0x01;

    /**
     * An enum representing data types that can be stored in a mesh
     */
    enum class TypeHint : uint8_t {
        R8UInt = 0,                // 0b00000000
        R8SInt = 1,                // 0b00000001
        R8SFloat = 2,              // 0b00000010 Technically not possible, but required for the strip type info stuff
    ...
    struct DatMeshHeader {
        uint8_t signature[8] = {};
        uint8_t version = 0;
        uint8_t vertexSize = 0;
        uint32_t vertexCount = 0;
        uint32_t indexCount = 0;
        uint8_t typeHintSize = 0;
    };

    /**
     * Strip the layout information from a type hint, getting the equivalent type 8 bit and with a single component
     *
     * @param typeHint The type hint to get the primitive of
     * @return The primitive type represented by the TypeHint
     */
    static TypeHint getPrimitiveTypeHint(const TypeHint typeHint) {
        return static_cast<TypeHint>(static_cast<uint8_t>(typeHint) & 0b00001111);
    }

    /**
     * Get the size in bits of each component in the given type
     *
     * @param typeHint The type hint to get the component size of
     * @return The component size of the Type in bits
     */
    static uint32_t getPrimitiveSize(const TypeHint typeHint) {
     ...
     *
     * @param typeHint The type hint to get the component count of
     * @return The number of components in the type
     */
    static uint32_t getComponentCount(const TypeHint typeHint) {
        return ((static_cast<uint32_t>(typeHint) & 0b00110000) >> 4) + 1;
    }
} // namespace DatAsset::DatMesh
